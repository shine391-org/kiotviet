...
if [[ "${1-}" == "--docker" ]]; then
  echo "🐳 Cleaning caches inside container ${DOCKER_CONTAINER:-kiotviet-web-1}..."
  docker exec ${DOCKER_CONTAINER:-kiotviet-web-1} sh -lc 'rm -rf writable/cache/* writable/logs/* && mkdir -p writable/cache writable/logs && chmod -R 777 writable' || true
fi
