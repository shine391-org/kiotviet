...
trait CashTransactionSchemaTrait
{
    ...
    {
        $db = $this->getSchemaDb();
        $required = ['cash_transactions', 'orders', 'purchase_orders', 'branches', 'users', 'returns'];
        $existing = array_flip($db->listTables());
        foreach ($required as $table) {
            if (! isset($existing[$table])) {
                throw new \RuntimeException("Missing required table {$table} in test DB; restore schema before running cash transaction tests.");
            }
        }

        $this->truncateTables($db, $required);
    }
