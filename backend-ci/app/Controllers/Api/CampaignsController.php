<?php

namespace App\Controllers\Api;

use App\Controllers\BaseController;
use App\Services\Campaigns\CampaignService;
use CodeIgniter\API\ResponseTrait;

/** Campaigns API. @agent-controller: Campaigns @agent-pattern: Thin controller */
class CampaignsController extends BaseController
{
    use ResponseTrait;

    protected CampaignService $service;

    public function __construct()
    {
        $this->service = service('campaignService');
    }

    /** Create campaign. @agent-use: POST /api/campaigns */
    public function create()
    {
        $payload = $this->request->getJSON(true) ?? [];
        return $this->wrap(fn () => $this->respondCreated($this->service->create($payload)));
    }

    /** Add member. @agent-use: POST /api/campaigns/{id}/members */
    public function addMember($id)
    {
        $payload = $this->request->getJSON(true) ?? [];
        return $this->wrap(fn () => $this->respond($this->service->addMember((int) $id, $payload)));
    }

    private function wrap(callable $action)
    {
        try { return $action(); }
        catch (\InvalidArgumentException $e) { return $this->failValidationErrors($e->getMessage()); }
        catch (\RuntimeException $e) { return $this->failNotFound($e->getMessage()); }
        catch (\Throwable $e) { return $this->failServerError($e->getMessage()); }
    }
}
