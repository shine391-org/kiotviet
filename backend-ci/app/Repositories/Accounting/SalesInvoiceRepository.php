...
use App\Models\SalesInvoiceItemModel;
use App\Models\SalesInvoiceTaxModel;
use App\Models\PaymentScheduleModel;
use CodeIgniter\Database\BaseConnection;

/**
 * @agent-repository: Sales invoices
 * @agent-pattern: Repository with children
 * @agent-reusable: MEDIUM
 */
class SalesInvoiceRepository
{
    protected SalesInvoiceModel $invoices;
    protected SalesInvoiceItemModel $items;
    protected SalesInvoiceTaxModel $taxes;
    protected PaymentScheduleModel $schedules;
    protected BaseConnection $db;

    public function __construct(
        ?SalesInvoiceModel $invoices = null,
        ?SalesInvoiceItemModel $items = null,
        ?SalesInvoiceTaxModel $taxes = null,
        ?PaymentScheduleModel $schedules = null,
        ?BaseConnection $db = null
    ) {
        $this->db = $db ?? \Config\Database::connect(ENVIRONMENT === 'testing' ? 'tests' : null);
        $this->invoices = $invoices ?? new SalesInvoiceModel();
        $this->items = $items ?? new SalesInvoiceItemModel();
        $this->taxes = $taxes ?? new SalesInvoiceTaxModel();
        $this->schedules = $schedules ?? new PaymentScheduleModel();
    }

    public function create(array $invoice, array $items, array $taxes, array $schedules): array
    {
        $now = $this->now();
        ...
        foreach ($schedules as $sch) {
            $scheduleRows[] = $sch + ['invoice_id' => $invoiceId, 'created_at' => $now, 'updated_at' => $now];
        }
        if ($scheduleRows) {
            $this->schedules->insertBatch($scheduleRows);
        }
    ...
    public function findById(int $id): ?array
    {
        ...
        $schedules = $this->schedules->where('invoice_id', $id)->findAll();
        ...
        $invoice['schedules'] = array_map(fn ($s) => $this->hydrateSchedule($s), $schedules);
        return $invoice;
    }
    ...
    private function hydrateSchedule(array $row): array
    {
        $row['id'] = isset($row['id']) ? (int) $row['id'] : null;
        $row['invoice_id'] = isset($row['invoice_id']) ? (int) $row['invoice_id'] : null;
        $row['amount'] = isset($row['amount']) ? (float) $row['amount'] : 0.0;
